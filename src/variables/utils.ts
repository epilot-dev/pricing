...
type PriceItemWithParent = PriceItem &
  CompositePriceItem & {
    parent_item: CompositePriceItem;
  };

type PriceDisplayType = 'show_as_on_request' | 'show_as_starting_price';

type GetTieredUnitAmountOptions = {
  isUnitAmountApproved: boolean;
  useUnitAmountNet: boolean;
};

interface ExternalFeesMetadata {
...
interface VariableFee extends BaseFee {
  /** Per unit amounts */
