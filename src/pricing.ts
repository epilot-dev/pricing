...
const getPriceComponents: GetPriceComponents = (priceItem) => {
  ...
  const ensureComponentWithValidPrice = (itemComponent: PriceItemDto) => ({
    ...itemComponent,
  ...
  return priceItem.item_components.map((itemComponent) => ({
...
export const computePriceItem: ComputePriceItem = (priceItem, price, applicableTax, quantity, priceMapping) => {
  ...
  switch (price?.pricing_model) {
    ...
    default:
      itemValues = computePriceItemValues(unitAmountDecimal, currency, isTaxInclusive, unitAmountMultiplier, priceTax);
  }
