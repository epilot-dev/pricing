...
import type { CompositePriceItem, PriceItem } from './types';
...
describe('Immutable data - computeAggregatedAndPriceTotals', () => {
  describe('with external pricing details', () => {
    it('returns the correct totals and items for simple price', () => {
      ...
      const oneTimeTotalRecurrence = totalDetailsBreakdown?.recurrences?.find(
        (recurrence) => recurrence.type === 'one_time',
      );
    ...
    it('returns the correct totals and items for composite price', () => {
      ...
      const monthlyTotalRecurrence = totalDetailsBreakdown?.recurrences?.find(
        (recurrence) => recurrence.type === 'recurring' && recurrence.billing_period === 'monthly',
      );
  ...
  describe('with internal and external pricing details', () => {
    it('returns the correct totals and items', () => {
      ...
      const simpleInternalOneTimePriceItem = computedItems?.find(
        (item) => item._price?._id === 'efe9ff76-865c-4287-8de9-422cfc741ff9',
      ) as PriceItem;
      const externalCompositePriceItem = computedItems?.find(
        (item) => item._price?._id === 'price-12312414',
      ) as CompositePriceItem;
      const simpleExternalOneTimePriceItem = computedItems?.find(
        (item) => item._price?._id === 'price-73f857a4-0fbc-4aa6-983f-87c0d6d410a6',
      ) as PriceItem;
      const totalDetailsBreakdown = result.total_details?.breakdown;
      ...
      const oneTimeExternalCompositePriceRecurrence = externalCompositePriceTotalDetailsBreakdown?.recurrences?.find(
        (recurrence) => recurrence.type === 'one_time',
      );
      const monthlyExternalCompositePriceRecurrence = externalCompositePriceTotalDetailsBreakdown?.recurrences?.find(
        (recurrence) => recurrence.type === 'recurring' && recurrence.billing_period === 'monthly',
      );
